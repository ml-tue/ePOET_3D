...
class CppnEnvParams:
    ...
    # y = np.array([(i - 200 / 2.0) / (200 / 2.0) for i in range(200)])
    x = np.array([((i - 20 / 2.0) / (20 / 2.0),(j - 20 / 2.0) / (20 / 2.0) ) for i in range(20) for j in range(20)])
    def __init__(self, cppn_config_path='config-cppn', genome_path=None):
        ...
        #self.altitude_fn = lambda x: np.square(x[0]**2+x[1]**2)
        self.altitude_fn = lambda x: np.cos(x[0]*x[1])
        if genome_path is not None:
            ...
            start_cppn_genome.nodes[0].activation = 'sin' # 'identity'
    ...
    def get_mutated_params(self):
        print('get_mutated_params starts')
        is_valid = False
        while not is_valid:
            # mutated = self.cppn_genome
            mutated = copy_genome(self.cppn_genome)
            mutated.nodes[0].response = 10.0
            mutated.key = datetime.datetime.utcnow().isoformat()
            mutated.mutate(self.cppn_config.genome_config)
            # print('distance', self.cppn_genome.distance(mutated, self.cppn_config.genome_config))
            is_valid = is_genome_valid(mutated) & (self.cppn_genome.distance(mutated, self.cppn_config.genome_config) > 0.8)
            if not is_valid:
                continue
            net = neat.nn.FeedForwardNetwork.create(mutated, self.cppn_config)
            y = np.array([net.activate((xi[0],xi[1])) for xi in self.x])
            y -= y[0] # normalize to start at altitude 0
            threshold_ = np.abs(np.max(y))
            is_valid = (threshold_ > 0.1 and np.var(y)>0.1)
            if not is_valid:
                continue
            if threshold_ < 0.25:
    ...
    def save_genome(self, filepath='/home/TUE/20191160/ant_poet/rs1/cppngenomes/', name=None):
        if name != None:
            file_path = filepath + name + '.pickle'
        else:
            file_path = filepath + 'genome_{}_saved.pickle'.format(time.time())
        # file_path = '/home/fang/project/thesis/halfcheetah_poet/locallog/cppngenomes/genome_{}_saved.pickle'.format(time.time())
        pickled = pickle.dump(self.cppn_genome, open(file_path, 'wb'))


def copy_genome(genome):
    file_path = '/home/TUE/20191160/ant_poet/tmp/genome_{}.pickle'.format(time.time())
