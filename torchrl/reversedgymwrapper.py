...
class ReverseGymWrapper(dm_env.Environment):
  ...
  def __init__(self, gym_env: gym.Env, num_episodes: int):

    super(dm_env.Environment, self).__init__()
    ...
    self.bsuite_num_episodes = num_episodes
  ...
  def step(self, action: int) -> dm_env.TimeStep:
    ...
    if done:
      is_truncated = False #for hex #info.get('TimeLimit.truncated', False)
      if is_truncated:
