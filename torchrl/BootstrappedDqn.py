...
class BootstrappedDqn(base.Agent):
  ...
  def __init__(
    ...
    # self._num_actions = action_spec.shape[0]
...
def default_agent(
  ...
  # Define network.
  prior_scale = 5.
  hidden_sizes = [64, 64] #[50, 50]

  def network(inputs: jnp.ndarray) -> jnp.ndarray:
    """Simple Q-network with randomized prior function."""
    net = hk.nets.MLP([*hidden_sizes, action_spec.shape[0]])
    prior_net = hk.nets.MLP([*hidden_sizes, action_spec.shape[0]])
    x = hk.Flatten()(inputs)
