...
class Hexapod():
    ...
    def __init__(self, env_list=None, max_n_envs=6, specific_env_len=30, s_len=200, walls=True):
        ...
        if self.use_HF:
            ...
            self.obs_dim = 18 * 2 + 6 + 4 + 6 + 1 + 2
        self.act_dim = self.sim.data.actuator_length.shape[0]

        self.action_space = Box(-1.0, 1.0, (18,), np.float32)
        self.observation_space = Box(-np.inf, np.inf, (55,), np.float64)
    ...
    #     return np.concatenate((self.scale_joints(qpos[7:]), qvel[6:], qpos[3:7], qvel[:6], contacts, r0, r1))
    ...
    def step(self, ctrl):
        ...
        if self.use_HF:
            ...
            #sensors = [self.sim.data.get_sensor('r0').tolist(), self.sim.data.get_sensor('r1').tolist()]
            no_intersection = -1.0
            sensors = [self.sim.data.get_sensor('r0').tolist(), self.sim.data.get_sensor('r1').tolist()]
            sensors = np.where(sensors == no_intersection, 1.0, np.tanh(sensors))
            obs = np.concatenate([self.scale_joints(self.sim.get_state().qpos.tolist()[7:]),
                                  self.sim.get_state().qvel.tolist()[6:],
                                  self.sim.get_state().qvel.tolist()[:6],
                                  [qw, qx, qy, qz, y],
