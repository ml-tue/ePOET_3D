...
#from .hexapod_trossen_terrain_all import Hexapod
def get_env(env_id, env_param):
    if env_id == "Hexapod":
        from .hexapod_trossen_terrain_all import Hexapod
        print('Hexapod env generated')
        env = Hexapod()
    elif env_id == "Hexapod-sensor":
        from .hexapod_sensor import Hexapod
        print('Hexapod with sensors env generated')
        env = Hexapod()
    else:
        env = gym.make(env_id)
...
def get_single_env(env_id, env_param):
    if env_id == "Hexapod":
        from .hexapod_trossen_terrain_all import Hexapod
        print('single Hexapod env generated')
        env = Hexapod()
    elif env_id == "Hexapod-sensor":
        from .hexapod_sensor import Hexapod
        print('single Hexapod with sensors env generated')
        env = Hexapod()
    else:
        env = gym.make(env_id)
    if str(env.__class__.__name__).find('TimeLimit') >= 0:
